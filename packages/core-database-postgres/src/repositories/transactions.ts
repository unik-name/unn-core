...
export class TransactionsRepository extends Repository implements Database.ITransactionsRepository {
    ...
    public async getAssetsByTypes(types: number[], typeGroup: number, limit: number, offset: number): Promise<any> {
        return this.db.manyOrNone(queries.stateBuilder.assetsByTypes, { typeGroup, types, limit, offset });
    }
    ...
    > {
        ...
        const types = activatedHandlers.map(h => ({
            type: h.getConstructor().type,
            typeGroup: h.getConstructor().typeGroup,
        }));
        for (const feeStatsByType of types) {
            const feeStatsForType = await this.db.oneOrNone(queries.transactions.feeStatisticsByType, {
