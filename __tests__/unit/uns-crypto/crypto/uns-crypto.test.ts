...
describe("uns-crypto.ts", () => {
    describe("validate DiscloseDemand signatures functions", () => {
        ...
        const certificationPayload: IDiscloseDemandCertificationPayload = {
            iss: ISS_UNIK_ID,
            sub: CERT_PAYLOAD_SUB,
            iat: 12345678,
        };

        const issKeys = Keys.fromPassphrase(ISS_PASSPHRASE);
        const demanderKeys = Keys.fromPassphrase(OWNER_PASSPHRASE);
